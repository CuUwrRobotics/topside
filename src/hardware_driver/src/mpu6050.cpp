...
custom_msgs::EulerMotion processMpu6050Data(std::uint8_t raw_buffer[])
{
    ...
    Vector3D gravity(
        (2
         * (gyro.b * gyro.d
            - gyro.a * gyro.c)), // Multiply by two to convert to 1g like above
    ...
    result.roll = std::atan(
        gravity.y / std::sqrt(gravity.x * gravity.x + gravity.z * gravity.z));
    result.pitch = std::atan(
        gravity.x / std::sqrt(gravity.y * gravity.y + gravity.z * gravity.z));
    result.yaw = std::atan2(2 * (gyro.b * gyro.c - gyro.a * gyro.d),
                            2 * (gyro.a * gyro.a - gyro.b * gyro.b) - 1);
