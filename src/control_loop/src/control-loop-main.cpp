...
void loop()
{
    ...
    if (desired_w == -999)
    {
        ...
        {
            // Set the yaw to the current value so the robot doesn't try to
            // pivot itself immediately
    ...
    {
        // Note that pi was chosen in the above line because it's half the
        // maximum change, so pi is the maximum distance from a small change
        // (eg. 0.1) and an overflow change (eg. 2pi - 0.1). Techinally other
        // values like 1 thru 6 would work, but they'd be slightly worse.
...
int main(int argc, char* argv[])
{
    ...
    // These handle communications to other nodes (the input/output of the
    // program)
    ros::Subscriber joy_sub = node.subscribe<custom_msgs::EulerMotion>(
        "robot_motion/controller_input",
        1,
        &controllerRosCallback);
    ros::Subscriber mpu_sub
        = node.subscribe<custom_msgs::EulerMotion>("hardware/motion",
                                                   1,
                                                   &accelGyroRosCallback);
    ros::Subscriber depth_sub
        = node.subscribe<std_msgs::Float32>("hardware/depth",
                                            1,
                                            &depthRosCallback);
    ros::Publisher output_vector_pub = node.advertise<custom_msgs::EulerMotion>(
        "robot_motion/control_loop_output",
        1);
    ros::ServiceServer lockout_service
        = node.advertiseService("lockout", &lockoutCallback);
