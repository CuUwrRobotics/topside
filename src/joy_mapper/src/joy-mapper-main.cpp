...
custom_msgs::ArmJointMotion arm_position_vector;
...
void initialize() {
  ...
  arm_position_vector.arm_joints[0] = 0;
  arm_position_vector.claw_open_close = 0;
...
int main(int argc, char* argv[]) {
  ...
  robot_motion_pub = node.advertise<custom_msgs::EulerMotion>(
      "robot_motion/controller_input", 1);
  arm_motion_pub = node.advertise<custom_msgs::ArmJointMotion>("arms/arm0", 1);
  ...
  while (ros::ok()) {
    ...
    if (new_joystick_data_ready) pushUpdates();
